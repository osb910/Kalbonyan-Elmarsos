...
export default function (number) {
  ...
  if (decimal == '99' && integer !== '0') {
    ...
    return `99/100`;
  } else if (1 - parseFloat(`.${decimal}`) < 0.0011) {
  ...
  if (pattern && decimal.length > 2) {
    ...
    if (endPattern.length > 1 && endPattern.length % 2 === 0) {
      ...
      endPattern =
        parseInt(endPattern.slice(0, endPattern.length / 2), 10) -
          parseInt(
            endPattern.slice(endPattern.length / 2, endPattern.length),
            10
          ) ===
        0
          ? endPattern.slice(0, endPattern.length / 2)
          : endPattern;
    }
...
function yesRepeat(decimal, endPattern, patternSequence, integer, type) {
  ...
  const nonPatternLength =
    decimal.length - patternSequence.length >= 1
      ? decimal.length - patternSequence.length
      : 1; //Does the length of the non pattern segment of the input = 0? If it does, that's incorrect since we know it must equal at least 1, otherwise it's the length of the decimal input minus the length of the full pattern.
  ...
  const numerator = Math.round(
    (float * decimalMultiplier1 - float) * Math.pow(10, nonPatternLength)
  ); //Find the numerator to be used in calculating the fraction that contains a repeating trailing sequence.
